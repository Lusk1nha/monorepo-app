...
impl AuthService {
    ...
    pub async fn login_with_credentials(&self, user_id: &str) -> Result<Session, AuthServiceError> {
    ...
    pub async fn validate_otp(&self, user_id: &str, code: &str) -> Result<(), AuthServiceError> {
        self.otp_codes_service
            .validate_otp_code(&user_id, &code)
            .await?;
